...
interface DashboardProps {
  widgets: WidgetData[];
  onUpdateWidget: (widget: WidgetData) => void;
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const updateWidgetData = (id: string, newData: WidgetData) => {
    onUpdateWidget(newData);
  };

  const setWidgetSize = (id: string, cols: number) => {
    const widget = widgets.find((w) => w.id === id);
    if (widget) {
      onUpdateWidget({ ...widget, cols });
    }
  ...
  const getIcon = (type: WidgetType) => {
    switch (type) {
      ...
      case WidgetType.KANBAN:
        return (
          <Kanban size={18} className="text-orange-500 dark:text-orange-400" />
        );
      case WidgetType.REMINDER:
        return <Bell size={18} className="text-rose-500 dark:text-rose-400" />;
      case WidgetType.GYM:
        return (
      ...
      case WidgetType.LINKS:
        return (
          <LinkIcon size={18} className="text-cyan-500 dark:text-cyan-400" />
        );
        ...
        return (
          <div
            ...
            className={`
              ...
              ${
                isEditMode
                  ? "border-indigo-400 dark:border-indigo-500 shadow-md ring-2 ring-indigo-500/10"
                  : "border-slate-200/60 dark:border-slate-800 hover:shadow-md"
              }
              ${isDragging ? "opacity-40 scale-95 border-dashed border-2" : ""}
              ${
                isOver && !isDragging
                  ? "translate-y-2 border-indigo-500 ring-4 ring-indigo-200 dark:ring-indigo-900"
                  : ""
              }
            `}
          >
            ...
            >
              <div className="flex items-center gap-2.5">
                {isEditMode && (
                  <GripVertical
                    size={16}
                    className="text-slate-400 dark:text-slate-500 mr-[-4px]"
                  />
                )}
                ...
                <h3
                  className="font-semibold text-slate-800 dark:text-slate-200 text-sm tracking-tight truncate max-w-[150px]"
                  title={widget.title}
                >
                  {widget.title}
                </h3>
                  ...
                  <>
                    ...
                    <div className="relative hidden sm:block">
                      <button
                        onClick={() =>
                          setActiveResizeMenu(
                            activeResizeMenu === widget.id ? null : widget.id
                          )
                        }
            ...
            <div
              className={`flex-1 p-4 overflow-hidden relative bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors rounded-b-2xl ${
