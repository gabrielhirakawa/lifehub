                ...
                <div className="flex items-center justify-between bg-indigo-50 dark:bg-indigo-900/20 p-2 rounded-lg border border-indigo-100 dark:border-indigo-900/50 sticky top-0 z-10 backdrop-blur-sm">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-red-500 text-xs hover:bg-red-50 p-1.5 rounded"
                    >
                      Cancel
                    </button>
                    <button
                ...
                {activeSession.logs.map((log) => (
                  ...
                  >
                    <div className="bg-slate-100 dark:bg-slate-800 p-2 text-xs font-semibold text-slate-700 dark:text-slate-200 flex justify-between items-center">
                      {log.exerciseName}
                      <button
                        onClick={() => addSet(log.exerciseName)}
                        className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-200 dark:hover:bg-indigo-900/50 p-1 rounded transition-colors"
                        title="Add Set"
                      >
                        <Plus size={14} />
                      </button>
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      ...
                      {log.sets.map((set, idx) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              toggleSetComplete(log.exerciseName, set.id)
                            }
                          ...
                          <button
                            onClick={() => deleteSet(log.exerciseName, set.id)}
            ...
            {isCreating ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-3 space-y-3 animate-in fade-in zoom-in-95">
                <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2">
                  <h4 className="font-semibold text-xs text-slate-700 dark:text-slate-200">
                    New Workout
                  </h4>
                  <button
                ...
                <div className="space-y-1">
                  {newExercises.map((ex, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center text-xs bg-slate-50 dark:bg-slate-800 p-1.5 rounded"
                    >
                      <span>{ex}</span>
                      <button
                        onClick={() => removeExerciseFromTemplate(i)}
                        className="text-slate-400 hover:text-red-500"
                      >
                        <X size={10} />
                      </button>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && addExerciseToTemplate()
                    }
                  />
                  <button
                    onClick={addExerciseToTemplate}
                    className="bg-slate-100 dark:bg-slate-700 p-1.5 rounded text-slate-600 hover:bg-slate-200"
                  >
                    <Plus size={14} />
                  </button>
                </div>
                <button
                  ...
                  className="w-full bg-indigo-600 text-white py-1.5 rounded text-xs font-medium hover:bg-indigo-700"
                >
                  Save Workout
                </button>
              </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  {templates.map((t) => (
                    <div
                      key={t.id}
                      className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800 flex justify-between items-start group"
                    >
                      <div>
