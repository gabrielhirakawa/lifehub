...
import { Plus, Trash2, ChevronLeft, ChevronRight, Apple, X, Settings } from 'lucide-react';
...
const DietWidget: React.FC<DietWidgetProps> = ({ data, onUpdate }) => {
  ...
  const [tempGoal, setTempGoal] = useState(data.content?.diet?.calorieGoal || 2000);
  ...
  const saveSettings = () => {
    onUpdate({
      ...data,
      content: { ...data.content, diet: { ...dietData, calorieGoal: tempGoal } }
    });
  ...
  const totalCalories = activeDay.meals.reduce((sum, meal) => 
    sum + meal.items.reduce((s, item) => s + item.calories, 0), 0);
  
  const totalProtein = activeDay.meals.reduce((sum, meal) => 
    ...
    <div className="h-full flex flex-col relative">
      ...
      {showSettings && (
        <div className="absolute inset-0 z-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 rounded-xl">
           <div className="w-full space-y-4">
              <h4 className="font-semibold text-slate-800 dark:text-slate-200">Diet Settings</h4>
              <div>
                <label className="text-xs text-slate-500 dark:text-slate-400 block mb-1">Daily Calorie Goal</label>
                <input 
                  type="number" 
                  value={tempGoal} 
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowSettings(false)} className="flex-1 bg-slate-100 dark:bg-slate-800 py-2 rounded text-xs">Cancel</button>
                <button onClick={saveSettings} className="flex-1 bg-lime-600 text-white py-2 rounded text-xs">Save</button>
              </div>
           </div>
        </div>
      )}
      ...
      <div className="flex-shrink-0 mb-3 space-y-2">
        <div className="flex justify-between items-center">
           <div className="flex items-center gap-1 text-sm font-semibold text-slate-700 dark:text-slate-200">
             <button onClick={() => changeDate(-1)} className="p-0.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded"><ChevronLeft size={16}/></button>
             <span>{activeDate === getTodayStr() ? 'Today' : activeDate.slice(5)}</span>
             <button onClick={() => changeDate(1)} className="p-0.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded"><ChevronRight size={16}/></button>
           </div>
           <button onClick={() => setShowSettings(true)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"><Settings size={14}/></button>
        </div>
        ...
        <div className="relative h-6 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden border border-slate-200 dark:border-slate-700">
           <div 
             className={`absolute top-0 left-0 h-full transition-all duration-500 ${isOver ? 'bg-red-500' : 'bg-lime-500'}`} 
        ...
        <div className="flex justify-center text-[10px] text-slate-400 gap-3">
           <span>Protein: <b className="text-slate-600 dark:text-slate-300">{totalProtein}g</b></span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
        {activeDay.meals.map(meal => (
          <div key={meal.id} className="bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800 p-2">
            <div className="flex justify-between items-center mb-2">
               <h5 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">{meal.name}</h5>
            ...
            <div className="space-y-1 mb-2">
              {meal.items.length === 0 && <div className="text-[10px] text-slate-400 italic pl-1">Empty</div>}
              {meal.items.map(food => (
                <div key={food.id} className="group flex justify-between items-center text-xs bg-white dark:bg-slate-800 p-1.5 rounded border border-slate-100 dark:border-slate-700">
                   <div className="flex-1 truncate pr-2">
                     <span className="text-slate-700 dark:text-slate-200">{food.name}</span>
                   </div>
                   <div className="flex items-center gap-2">
                     <span className="text-slate-500 dark:text-slate-400 font-medium">{food.calories}</span>
                     <button onClick={() => removeFood(meal.id, food.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><X size={12}/></button>
                   </div>
                </div>
              ))}
            </div>
            ...
            {isAddingToMeal === meal.id ? (
              <div className="bg-white dark:bg-slate-900 p-2 rounded border border-indigo-200 dark:border-indigo-800 animate-in fade-in zoom-in-95">
                 ...
                 <div className="flex gap-2">
                    <button onClick={() => setIsAddingToMeal(null)} className="flex-1 bg-slate-100 dark:bg-slate-800 py-1 rounded text-[10px] hover:bg-slate-200">Cancel</button>
                    <button onClick={addFood} className="flex-1 bg-indigo-600 text-white py-1 rounded text-[10px] hover:bg-indigo-700">Add</button>
                 </div>
              </div>
            ) : (
              <button 
                onClick={() => { setIsAddingToMeal(meal.id); setFoodName(''); setCalories(''); setProtein(''); }}
