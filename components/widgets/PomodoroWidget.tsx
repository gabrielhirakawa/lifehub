...
const MODES: Record<PomodoroMode, { label: string; minutes: number; color: string; icon: any }> = {
  work: { 
    label: 'Focus', 
    minutes: 25, 
...
const PomodoroWidget: React.FC<PomodoroWidgetProps> = ({ data, onUpdate }) => {
  ...
  const { timeLeft = MODES.work.minutes * 60, isActive = false, mode = 'work', cyclesCompleted = 0, endTime } = data.content?.pomodoro || {};
    ...
    if (isActive && endTime) {
      ...
      const tick = () => {
        ...
        if (secondsRemaining <= 0) {
          ...
          const newCycles = mode === 'work' ? cyclesCompleted + 1 : cyclesCompleted;
  ...
  const toggleTimer = () => {
    if (isActive) {
      ...
      const now = Date.now();
      const currentRemaining = endTime ? Math.max(0, Math.ceil((endTime - now) / 1000)) : timeLeft;
  ...
  return (
    <div className="h-full flex flex-col items-center justify-between py-2">
      ...
      {/* Mode Selector */}
      <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg w-full mb-4">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative w-full">
        ...
        <div className={`w-40 h-40 rounded-full flex items-center justify-center border-4 transition-colors relative ${
            isActive ? 'border-slate-200 dark:border-slate-700' : 'border-slate-100 dark:border-slate-800'
        }`}>
           <div className={`text-4xl font-mono font-bold tracking-wider ${isActive ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400 dark:text-slate-500'}`}>
             {formatTime(displayTime)}
           </div>
           
           {/* Mode Icon floating */}
           <div className={`absolute -top-3 bg-white dark:bg-slate-900 p-1.5 rounded-full border shadow-sm ${MODES[mode].color.split(' ')[0]}`}>
              <CurrentIcon size={20} />
           </div>
        </div>
        
        <p className={`mt-4 text-xs font-medium px-3 py-1 rounded-full ${MODES[mode].color}`}>
      ...
      <div className="w-full flex items-center justify-center gap-4 mt-2">
        <button 
          onClick={resetTimer}
          className="p-3 rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 transition-colors"
          title="Reset"
        >
          <RotateCcw size={20} />
        </button>
        ...
        <button 
          onClick={toggleTimer}
          className={`p-4 rounded-full text-white shadow-lg transform transition-all active:scale-95 ${
            isActive 
      ...
      <div className="w-full text-center mt-2 text-[10px] text-slate-400 uppercase tracking-widest font-bold">
