...
import { Send, Sparkles, Loader2, Settings, X, Save, Globe } from 'lucide-react';
...
const AICoachWidget: React.FC<AICoachWidgetProps> = ({ allWidgets, data, onUpdate }) => {
  ...
  const messages = data.content?.chatHistory || [
    { role: 'model', text: config.language === 'pt-br' 
  ...
  return (
    <div className="h-full flex flex-col relative">
      ...
      {showSettings ? (
           ...
           <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar px-1">
             ...
             <div>
               ...
               <div className="flex gap-2">
                 <button 
                   onClick={() => setTempConfig({...tempConfig, language: 'pt-br'})}
                   className={`flex-1 py-1.5 text-xs rounded-md border transition-all ${tempConfig.language === 'pt-br' ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500 text-indigo-700 dark:text-indigo-400 font-semibold' : 'border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700'}`}
                 >
                   🇧🇷 PT-BR
                 </button>
                 <button 
                   onClick={() => setTempConfig({...tempConfig, language: 'en-us'})}
                   className={`flex-1 py-1.5 text-xs rounded-md border transition-all ${tempConfig.language === 'en-us' ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500 text-indigo-700 dark:text-indigo-400 font-semibold' : 'border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700'}`}
                 >
                   🇺🇸 EN-US
                 </button>
               </div>
             ...
             <div>
               ...
               <div className="grid grid-cols-3 gap-2">
                 <button 
                   onClick={() => setTempConfig({...tempConfig, provider: 'gemini', model: 'gemini-2.5-flash'})}
                   className={`py-2 text-xs rounded-md border transition-all ${tempConfig.provider === 'gemini' ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-500 text-blue-700 dark:text-blue-400 font-semibold' : 'border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700'}`}
                 >
                   Gemini
                 </button>
                 <button 
                   onClick={() => setTempConfig({...tempConfig, provider: 'openai', model: 'gpt-4o-mini'})}
                   className={`py-2 text-xs rounded-md border transition-all ${tempConfig.provider === 'openai' ? 'bg-green-50 dark:bg-green-900/30 border-green-500 text-green-700 dark:text-green-400 font-semibold' : 'border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700'}`}
                 >
                   OpenAI
                 </button>
                 <button 
                   onClick={() => setTempConfig({...tempConfig, provider: 'anthropic', model: 'claude-3-haiku-20240307'})}
                   className={`py-2 text-xs rounded-md border transition-all ${tempConfig.provider === 'anthropic' ? 'bg-orange-50 dark:bg-orange-900/30 border-orange-500 text-orange-700 dark:text-orange-400 font-semibold' : 'border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700'}`}
                 >
                   Anthropic
                 </button>
               </div>
             ...
             <div>
               ...
               <input 
                 ...
                 placeholder={tempConfig.provider === 'gemini' ? "Optional (Uses default env key)" : "sk-..."}
                 className="w-full text-xs p-2 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none"
               />
             </div>
           </div>
        ...
        <>
          {/* Chat Interface */}
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 mb-3 pt-6">
          ...
          <form onSubmit={handleSend} className="relative flex-shrink-0">
            <input
              ...
              placeholder={config.language === 'pt-br' ? `Perguntar ao ${getProviderName(config.provider)}...` : `Ask ${getProviderName(config.provider)}...`}
