...
const buildDashboardContext = (widgets: WidgetData[], lang: AILanguage = 'en-us') => {
  const t = TERMS[lang];
  ...
  widgets.forEach(w => {
    ...
    if (w.type === WidgetType.TODO) {
      ...
      const pending = activeTodos.filter(t => !t.completed).map(t => t.text).join(", ");
      if (pending) dashboardContext += `  ${t.pendingTasks}: ${pending}\n`;
      const archivedCount = w.content?.todos?.filter(t => t.archived).length || 0;
      if (archivedCount > 0) dashboardContext += `  (${archivedCount} ${t.archivedTasks})\n`;
      ...
      const amountStr = amount >= 1000 ? `${(amount/1000).toFixed(2)}L` : `${amount}ml`;
      ...
      if (notes.length > 0) {
        ...
        dashboardContext += `  ${t.noteContent}: ${w.content?.text?.substring(0, 100)}...\n`;
      }
    
    } else if (w.type === WidgetType.REMINDER) {
      const reminders = w.content?.reminders?.filter(r => !r.completed) || [];
      if (reminders.length > 0) {
        dashboardContext += `  ${t.upcomingReminders}: ${reminders.map(r => `${r.text} on ${r.date}`).join("; ")}\n`;
      } else {
...
const callAnthropic = async (apiKey: string, model: string, system: string, userMsg: string) => {
...
const callGemini = async (apiKey: string, model: string, system: string, context: string, userQuery?: string) => {
...
export const getGeminiInsight = async (widgets: WidgetData[], userQuery?: string, config?: AIConfig): Promise<string> => {
  ...
  const model = config?.model || (provider === 'openai' ? 'gpt-4o-mini' : (provider === 'anthropic' ? 'claude-3-haiku-20240307' : 'gemini-2.5-flash'));
    ...
    switch (provider) {
      case 'openai':
        const messages = [
          ...
          { role: "user", content: userQuery || (lang === 'pt-br' ? "Analise meu painel." : "Analyze my dashboard.") }
        ];
        return await callOpenAI(apiKey, model, messages) || "No response.";

      case 'anthropic':
        const anthropicMsg = `${context}\n\n${userQuery || (lang === 'pt-br' ? "Analise meu painel." : "Analyze my dashboard.")}`;
        return await callAnthropic(apiKey, model, systemPrompt, anthropicMsg) || "No response.";

      case 'gemini':
      default:
        return await callGemini(apiKey, model, systemPrompt, context, userQuery) || "No response.";
    }
  } catch (error: any) {
    console.error("AI Service Error:", error);
    return `Error (${provider}): ${error.message || "Failed to connect to AI service."}`;
  }
};